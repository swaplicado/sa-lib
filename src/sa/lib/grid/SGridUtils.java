...
public abstract class SGridUtils {
    ...
    public static final SGridCellRendererDate CellRendererDateGui = new SGridCellRendererDate(SLibUtils.GuiFormatDate);
                    ...
                    case SLibConsts.DATA_TYPE_DATE:
                        ...
                        switch (colType) {
                            ...
                            case SGridConsts.COL_TYPE_DATE_TIME:
                                ...
                                break;
                            case SGridConsts.COL_TYPE_DATE_GUI:
                                dateFormat = SLibUtils.GuiFormatDate;
                                break;
                            default:
                        }
                ...
                for (row = (gridPane.getTable().getSelectedRow()); row < rows; row++) {
                    ...
                    for (col = gridPane.getTable().getSelectedColumn() + 1; col <= cols; col++) {
                        if (col == cols) {
                            col = 0;
                            int auxRox = row + 1;
                            gridPane.setSelectedGridColumn(0);
                            if (auxRox != rows) {
                                if (gridPane.getTable().getValueAt(auxRox, col) != null) {
                                     if (gridPane.getTable().getValueAt(auxRox, col).toString().toUpperCase().contains(valueToSearchUpper)) {
                                        gridPane.setSelectedGridRow(auxRox);
